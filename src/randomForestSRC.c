...
char getBestSplitNew(uint         treeID,
                     Node        *parent,
                     uint        *repMembrIndx,
                     uint         repMembrSize,
                     uint        *allMembrIndx,
                     ...
                     SplitInfo  **splitInfo) {
  char result;
  genericSplitNew = &regressionXwghtSplitGreedy;
  result = genericSplitNew(treeID,
                           parent,
                           repMembrIndx,
                           repMembrSize,
                           allMembrIndx,
  ...
  if (flag) {
    if (*maxInfo == NULL) {
      *maxInfo = makeSplitInfo(info -> size);
      makePartialSplitInfo(*maxInfo, info -> hcDim);
    }
    else {
      freePartialSplitInfo(*maxInfo);
      makePartialSplitInfo(*maxInfo, info -> hcDim);
    }
...
void freeSplitInfo(SplitInfo *info) {
  ...
  free_uivector(info -> mwcpSizeAbs, 1, info -> hcDim);
  free_uivector(info -> randomVar, 1, info -> hcDim);
  free_new_vvector(info -> randomPts, 1, info -> hcDim, NRUTIL_VPTR);
  free_gblock(info, (size_t) sizeof(SplitInfo));
}
SplitInfo *makePartialSplitInfo(SplitInfo *info, uint hcDim) {
  uint hcDimAdj;
  info -> hcDim     = hcDim;
  info -> hcPartDim = 0;
  info -> hcPartIdx = 0;
...
void freePartialSplitInfo(SplitInfo *info) {
  uint k;
  uint hcDim;
  hcDim = info -> hcDim;
  if (hcDim == 0) {
...
void getSplitObjectInfo(void *info) {
  RF_nativePrint("\nSplitInfo:  %20x \n", info);
  RF_nativePrint("\nSplitVirtualInfo:  %20x \n", info);
}
...
char forkAndUpdateNew(uint       treeID,
                      Node      *parent,
                      uint      *repMembrIndx,
                      uint       repMembrSize,
                      uint      *allMembrIndx,
  ...
  char result;
  uint i, j;
  result = forkNodeNew(parent, info);
  if (result == TRUE) {
...
char growTree (uint     r,
  ...
  if (bootResult) {
    ...
    if (RF_htry == 0) {
      ...
      splitResult = getBestSplitNew(treeID,
                                    parent,
                                    bootMembrIndx,
                                    bootMembrSize,
                                    allMembrIndx,
    ...
    if (splitResult == TRUE) {
      ...
      if (RF_htry == 0) {
        ...
        forkResult = forkAndUpdateNew(treeID,
                                      parent,
                                      bootMembrIndx,
                                      bootMembrSize,
                                      allMembrIndx,
...
void saveTree(uint    b,
  ...
  RF_parmID_[*offset] = parent -> splitParameter;
  if ((parent -> splitParameter) != 0) {
    if (RF_xType[parent -> splitParameter] == 'C') {
      RF_mwcpSZ_[*offset] = parent -> splitValueFactSize;
      for (i = 1; i <= RF_mwcpSZ_[*offset]; i++) {
        (*mwcpPtr) ++;
        **mwcpPtr = (parent -> splitValueFactPtr)[i];
        RF_mwcpCount[b] ++;
      }
      RF_contPT_[*offset] = RF_nativeNaN;
    }
    else {
      RF_contPT_[*offset] = parent -> splitValueCont;
      RF_mwcpSZ_[*offset] = 0;
    }
  }
  else {
  ...
  (*offset) ++;
  if (((parent -> left) != NULL) && ((parent -> right) != NULL)) {
    saveTree(b, parent ->  left, offset, mwcpPtr);
    saveTree(b, parent -> right, offset, mwcpPtr);
  }
}
